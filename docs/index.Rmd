...
The `fig.ext='svg'` chunk option is useful html output but cannot produce italicised characters. The `tikz-template.tex` file includes the tikz preamble code from Eleanor Murray's LaTeX code plus the required template code. The contents of the template file is as follows.
...
```{tikz, fig.ext='png', fig.width=3, engine.opts=list(template = "tikz-template.tex")}
\begin{tikzpicture}
\node (1) at (0,0) {G};
\node (2) at (1.5,0) {X};
\node (3) at (3,0) {Y};
...
```{tikz, fig.ext='svg', fig.width=3, engine.opts=list(template = "tikz-template-swig-tr.tex")}
\begin{tikzpicture}
