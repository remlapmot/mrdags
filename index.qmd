...
```{r}
#| label: dg-single-dag
#| fig-alt: Single genotype DAG created with the DiagrammeR package.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-single-dag-2
#| fig-alt: Single genotype DAG without borders around variables.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-multiple-dag
#| fig-alt: Multiple genotype DAG created with the DiagrammeR package.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-multiple-dag-2
#| fig-alt: Multiple genotype DAG without borders around observed variables.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-bidir-fig-1
#| fig-alt: Bidirectional DAG 1.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: bg-bidir-fig-2
#| fig-alt: Bidirectional DAG 2.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-bidir-fig
#| fig-alt: Combined DAGs representing bidirectional MR.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-pleio-1
#| fig-alt: DAG for horizontal pleiotropy that causes bias.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-pleio-2
#| fig-alt: DAG for horizontal pleiotropy that does not cause bias.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-pleio-3
#| fig-alt: DAG for confounding via linkage disequilibrium.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-pleio-4
#| fig-alt: DAG for vertical pleiotropy.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-pleio-5
#| fig-alt: DAG for misspecification of the primary phenotype.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-pleio-6
#| fig-alt: DAG for correlated pleiotropy.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-single-path
#| fig-alt: Single genotype path diagram including the confounder.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-single-path-variance
#| fig-alt: Single genotype path diagram including variance terms.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dg-single-path-2
#| fig-alt: Single genotype path diagram including correlated error terms between exposure and outcome.
DiagrammeR::grViz("
  digraph mrdag {
...
```{r}
#| label: dgtty-single-dag
#| fig-alt: Single genotype DAG using DOT syntax.
#| fig-height: 2
mrdag <- dagitty('dag {
  G -> X -> Y
  X <- U -> Y
...
```{r}
#| label: ggdg-single-dag
#| warning: false
#| message: false
#| fig-alt: Single genotype DAG using ggdag.
#| fig-height: 3.5
library(ggdag)
mrdag <- dagitty('dag {
  G -> X -> Y
...
```{r}
#| label: ggdg-single-dag-2
#| fig-height: 3.5
#| fig-alt: Single genotype DAG created with ggdag with labels.
mrdag <- mrdag |>
  dag_label(
    labels = c(
      "X" = "Exposure",
...
```{r}
#| label: ggdg-single-dag-3
#| fig-height: 3.5
#| fig-alt: Identifying the instrumental variable using ggdag and dagitty.
mrdag |> ggdag_instrumental() + theme_dag()
```

It correctly identifies the genotype, $G$, as the instrumental variable.

### Using the dagitty web interface

We can draw the MR DAG in the dagitty web interface <https://www.dagitty.net/dags.html>.

```{r}
#| echo: false
#| fig-alt: The MR DAG created in the dagitty web interface.
knitr::include_graphics("img/dagitty-net-mr-dag.png")
```

The code is very similar to that above, except that the position values are different.

```dot
dag {
  bb="0,0,1,1"
  "Instrumental variable" [pos="0.150,0.200"]
  Confounder [latent,pos="0.600,0.100"]
  Exposure [exposure,pos="0.400,0.200"]
  Outcome [outcome,pos="0.800,0.200"]
  "Instrumental variable" -> Exposure
  Confounder -> Exposure
  Confounder -> Outcome
  Exposure -> Outcome
}
...
And dagitty also gives the tikz code.

```latex
% This code uses the tikz package
\begin{tikzpicture}
\node (v0) at (0.150,-0.200) {Instrumental variable};
\node (v1) at (0.600,-0.100) {Confounder};
\node (v2) at (0.400,-0.200) {Exposure};
\node (v3) at (0.800,-0.200) {Outcome};
\draw [->] (v0) edge (v2);
\draw [->] (v1) edge (v2);
\draw [->] (v1) edge (v3);
\draw [->] (v2) edge (v3);
...
This DAG is saved and available at <https://dagitty.net/dags.html?id=q2va7XkL>.

### Using R model type syntax in ggdag

```{r}
#| label: ggdg-single-dag-4
#| fig-height: 3.5
#| fig-alt: Specifying the DAG in ggdag with R model syntax.
...
```{r}
#| label: ggdg-single-dag-5
#| fig-height: 3.5
#| fig-alt: Plotting directly with ggplots functions.
mrdag |>
  ggplot(aes(
    x = x,
    y = y,
...
```{r}
#| label: ggdg-single-dag-6
#| fig-height: 3.5
#| fig-alt: Showing the paths on the DAG.
set.seed(123)
dagify(Y ~ X + U,
       X ~ U + G,
       exposure = "X",
...
```{r}
#| label: ggdg-single-dag-7
#| fig-height: 3.5
#| fig-alt: Showing the adjustment set.
dagify(Y ~ X + U,
       X ~ U + G,
       exposure = "X",
       outcome = "Y",
...
```{r} 
#| label: ggdg-multiple-dag
#| fig-alt: Mulitple genotype DAG.
...
%|   template: "tikz-template.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "The MR DAG created with TikZ."
\begin{tikzpicture}
\node (1) at (0,0) {$G$};
\node (2) at (1.5,0) {$X$};
\node (3) at (3,0) {$Y$};
...
%|   template: "tikz-template.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "Plotting the MR DAG using tikz-cd."
\usetikzlibrary{cd}
...
%| engine-opts:
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "The MR DAG created using simpler tikz code."
...
%|   template: "tikz-template.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "The multiple genotype DAG created using TikZ."
\begin{tikzpicture}
\node (1) at (0,0) {$G_2$};
\node (2) at (1.5,0) {$X$};
\node (3) at (3,0) {$Y$};
...
%|   template: "tikz-template.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "The multiple genotype DAG with circularly spaced genotypes."
\begin{tikzpicture}
\node (1) at (0,0) {$G_2$};
\node (2) at (1.5,0) {$X$};
\node (3) at (3,0) {$Y$};
...
%| engine-opts:
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "The horizontal pleiotropy DAG created using TikZ."
\usetikzlibrary{positioning}
\tikzset{
    > = stealth,
    every path/.append style = {
...
%| engine-opts:
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "The DAG for horizontal pleiotropy which causes no bias created using TikZ."
\usetikzlibrary{positioning}
\tikzset{
    > = stealth,
    every path/.append style = {
...
%| engine-opts:
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "The DAG for confounding via linkage disequilibrium created using TikZ."
\usetikzlibrary{positioning}
\tikzset{
    > = stealth,
    every path/.append style = {
...
%| engine-opts:
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "The DAG for vertical pleiotropy created using TikZ."
\usetikzlibrary{positioning}
\tikzset{
    > = stealth,
    every path/.append style = {
...
%| engine-opts:
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "MR DAG showing misspecification of the primary phenotype created using TikZ."
\usetikzlibrary{positioning}
\tikzset{
    > = stealth,
    every path/.append style = {
...
%| engine-opts:
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "The DAG for correlated pleiotropy created using TikZ."
\usetikzlibrary{positioning}
\tikzset{
    > = stealth,
    every path/.append style = {
...
%|   template: "tikz-template.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "One of the DAGs required for the bidirectional MR figure."
\begin{tikzpicture}
\node (1) at (0,0) {$G_X$};
\node (2) at (1.5,0) {$X$};
\node (3) at (3,0) {$Y$};
...
%|   template: "tikz-template.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "The reverse direction DAG required for the bidirectional MR figure."
\begin{tikzpicture}
\node (2) at (1.5,0) {$X$};
\node (3) at (3,0) {$Y$};
\node (4) at (2.25,1) {$U$};
...
%|   template: "tikz-template.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "The bidirectional MR figure combining both DAGs."
\begin{tikzpicture}
\node (1) at (0,0) {$G_X$};
\node (2) at (1.5,0) {$X$};
\node (3) at (3,0) {$Y$};
...
%|   template: "tikz-template.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "MVMR DAG 1 created using TikZ."
\begin{tikzpicture}
\node (1) at (0,0) {$G_2$};
\node (2) at (1.5,0) {$X_2$};
\node (3) at (4.5,1) {$Y$};
...
%|   template: "tikz-template.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "MVMR DAG 2 created using TikZ."
\begin{tikzpicture}
\node (1) at (0,0) {$G_2$};
\node (2) at (1.5,0) {$X_2$};
\node (3) at (4.5,1) {$Y$};
...
%|   template: "tikz-template.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "MVMR DAG 3 created using TikZ."
\begin{tikzpicture}
\node (1) at (0,0) {$G_2$};
\node (2) at (1.5,0) {$X_2$};
\node (3) at (4.5,1) {$Y$};
...
%|   template: "tikz-template.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "MVMR DAG 4 created using TikZ."
\begin{tikzpicture}
\node (1) at (0,0) {$G_2$};
\node (2) at (1.5,0) {$X_2$};
\node (3) at (4.5,1) {$Y$};
...
```{r}
#| label: quckdg-single-dag
#| message: false
#| warning: false
#| out-width: 65%
#| fig-alt: The single genotype DAG created using quickdag.
...
```{r}
#| label: quckdg-single-swig
#| out-width: 65%
#| fig-alt: The single genotype SWIG created using quickdag.
swig <- dag |>
  qd_swig(fixed_nodes = c("G", "X"))
swig |> render_graph()
```
...
### The simple SWIG for confounding

```{tikz swig-conf-color}
%| engine-opts:
%|   template: "tikz-template-swig-tr.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| out-width: "65%"
%| fig-alt: "The simple SWIG for confounding."
\begin{tikzpicture}
...
\node[name=X,
  ...
  swig vsplit = {line color right=red,
                 line width right=2.5pt}]{
\nodepart{left}{$X$}
\nodepart{right}{\textcolor{red}{$x$}}};
...
%|   template: "tikz-template-swig-tr.tex"
%|   dvisvgm.opts: "--font-format=woff"
%| fig-alt: "The SWIG for the MR DAG."
\begin{tikzpicture}
\tikzset{line width=1.5pt, outer sep=0pt,
  ell/.style={draw, fill=white, inner sep=2pt,
  line width=1.5pt},
...
%|   dvisvgm.opts: "--font-format=woff"
%| out-width: "65%"
%| fig-alt: "The SWIG for the MR DAG with colour."
\begin{tikzpicture}
\tikzset{line width=1.5pt, outer sep=0pt,
  ell/.style={draw, fill=white, inner sep=2pt,
  line width=1.5pt},
...
%|   dvisvgm.opts: "--font-format=woff"
%| out-width: "65%"
%| fig-alt: "The SWIG for the MVMR model with 2 independent exposures."
\begin{tikzpicture}
\tikzset{line width=1.5pt, outer sep=0pt,
  ell/.style={draw, fill=white, inner sep=2pt,
  line width=1.5pt},
...
%|   dvisvgm.opts: "--font-format=woff"
%| out-width: "65%"
%| fig-alt: "The SWIG for an MVMR model with confounding."
\begin{tikzpicture}
\tikzset{line width=1.5pt, outer sep=0pt,
  ell/.style={draw, fill=white, inner sep=2pt,
  line width=1.5pt},
...
%|   dvisvgm.opts: "--font-format=woff"
%| out-width: "65%"
%| fig-alt: "The SWIG for the MVMR model with colliders."
\begin{tikzpicture}
\tikzset{line width=1.5pt, outer sep=0pt,
  ell/.style={draw, fill=white, inner sep=2pt,
  line width=1.5pt},
...
%|   dvisvgm.opts: "--font-format=woff"
%| out-width: "65%"
%| fig-alt: "The SWIG for the MVMR model with mediation."
\begin{tikzpicture}
\tikzset{line width=1.5pt, outer sep=0pt,
  ell/.style={draw, fill=white, inner sep=2pt,
  line width=1.5pt},
...
```{r}
#| echo: false
#| fig-alt: The MR DAG created using excalidraw.
...
```{r}
#| echo: false
#| fig-alt: A second version of the MR DAG created using excalidraw.
